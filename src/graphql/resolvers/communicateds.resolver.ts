...
export class CommunicatedsResolver {
  ...
  async findCommunicateds(@Args('input') input: CommunicatedFilterInput): Promise<ICommunicated[]> {
    return this.communicatedsService.findAll(input);
  }

  @Query(() => CommunicatedType)
  async findOneCommunicated(@Args('input') input: CommunicatedFilterInput): Promise<ICommunicated> {
    return this.communicatedsService.findOne(input);
  }

  @Mutation(() => CommunicatedType)
  async upsertCommunicated(@Args('input') input: CommunicatedInput): Promise<ICommunicated> {
